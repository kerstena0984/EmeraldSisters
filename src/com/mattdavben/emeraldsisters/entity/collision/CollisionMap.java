...
public class CollisionMap {
	...
	public CollisionMap(TiledMap map) {
		...
		quadtree = new QuadTree(0, new Rectangle(0, 0, map.getWidth()
				* Environment.TILE_WIDTH, map.getHeight()
	...
	public void update(Player player, int delta) {
		...
		float distance = Player.WALKING_SPEED * Environment.TILE_WIDTH * delta
				/ 1000f;

		Rectangle completelyArbitraryRectangle = new Rectangle((int) 21.5 * 32,
				(int) 25.5 * 32, 16, 16);
		...
		Rectangle tinyRoomRectangle = new Rectangle((int) 11.5 * 32,
				(int) 18.5 * 32, 16, 16);
		...
		quadtree.retrieve(collidableShapesNearPlayer, player
				.getCollisionShape());

		for (int k = 0; k < collidableShapesNearPlayer.size(); k++) {
			if (!collidableShapesNearPlayer.get(k).equals(
					player.getCollisionShape())) {
				if (collisionShapeUp.intersects(collidableShapesNearPlayer
						.get(k))) {
					player.blockedUp = true;
				}
				if (collisionShapeDown.intersects(collidableShapesNearPlayer
						.get(k))) {
					player.blockedDown = true;
				}
				if (collisionShapeLeft.intersects(collidableShapesNearPlayer
						.get(k))) {
					player.blockedLeft = true;
				}
				if (collisionShapeRight.intersects(collidableShapesNearPlayer
						.get(k))) {
		...
		for (int layer = 0; layer < map.getLayerCount(); layer++) {
			for (int x = 0; x < map.getWidth(); x++) {
				for (int y = 0; y < map.getHeight(); y++) {
					...
					String blocked = map.getTileProperty(tileID, "blocked",
							"false");
					if (blocked.equals("true")) {
						String startingXString = map.getTileProperty(tileID,
								"startingXOffset", "0");
						String startingYString = map.getTileProperty(tileID,
								"startingYOffset", "0");
						String collisionWidth = map.getTileProperty(tileID,
								"collisionWidth", "32");
						String collisionHeight = map.getTileProperty(tileID,
								"collisionHeight", "32");
						...
						Rectangle collision = buildCollisionBox(tileID, x
								- startX, y - startY, width, height);
						SolidTile tile = new SolidTile(new Vector2f(x - startX,
								y - startY), collision);
	...
	private Rectangle buildCollisionBox(int tileID, int x, int y, int width,
			int height) {
